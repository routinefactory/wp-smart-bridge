...
(function ($) {
    ...
    */
    ...
    function initSettingsPage() {
        ...
        // 9.9 Static HTML Backup (v3.3.0)
        $('#sb-generate-static-backup').on('click', function () {
            var $btn = $(this);
            var $ui = $('#sb-static-backup-ui');
            var $status = $('#sb-static-backup-status-text');
            var $percent = $('#sb-static-backup-percent');
            var $bar = $('.sb-progress-bar-fill');
            var $download = $('#sb-static-backup-download');

            if (!confirm('전체 백업을 생성하시겠습니까? 링크 수에 따라 시간이 소요될 수 있습니다.')) {
                return;
            }
            ...
            function processBatch() {
                $.post(sbAdmin.ajaxUrl, {
                    ...
                    if (response.success) {
                        ...
                        var progress = 0;
                        if (total > 0) {
                            progress = Math.min(100, Math.round((response.data.processed / limit) * 100)); // This logic is wrong for total progress
                            // Correct logic for TOTAL progress
                            // Server returns 'offset' as NEXT offset. So processed count is 'offset'. (Actually offset is start, but response.data.offset is next start)
                            // response.data.offset IS the cumulative count processed so far (in loop) if we trust logic.
                            progress = Math.min(100, Math.round((offset / total) * 100));
                        }

                        $bar.css('width', progress + '%');
                        $percent.text(progress + '%');
                        $status.text(offset + ' / ' + total + ' 처리 완료');
                        ...
                        if (!response.data.finished) {
                            ...
                            // Finish
                            $status.text('백업 생성 완료!');
                            $btn.prop('disabled', false).removeClass('sb-spin');
                            $bar.css('width', '100%');

                            var downloadHtml = '<a href="' + response.data.download_url + '" class="button button-primary button-large" download>' +
                                '<span class="dashicons dashicons-download" style="line-height:28px;"></span> 백업 파일 다운로드 (.zip)' +
                                '</a>' +
                                '<p class="description" style="margin-top:10px;">파일을 다운로드 후 안전한 곳에 보관하세요.</p>';

                            $download.html(downloadHtml).fadeIn();
                        }
                    } else {
